...
class googleadd3pre_Dataset(data.Dataset):
    ...
    def __getitem__(self,index):
        ...
        add3pre=glob(f'data/samples/Syria_samples/samples_addpre/samples/{city}_*{coord}.png')
        if len(add3pre)!=3:
...
class googleadd3presize6_Dataset(data.Dataset):
    ...
    def __getitem__(self,index):
        ...
        city=basename.split('_')[0]
        coord=basename.split('_')[-1].split('.')[0]
        add3pre=glob(f'data/samples/Syria_samples/samples_addpre_size6_AREA/samples/{city}_*{coord}.pth')
        if len(add3pre)!=3:
            raise('add pre img num !=3')
        add3pre.sort()
        add3pre.append(path_pre)
